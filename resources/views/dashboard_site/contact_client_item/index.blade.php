                                            ...
                                            <tbody>
                                                @forelse ($deleted_client_data as $deleted_client )
                                                    <tr>
                                                        <td>{{$loop->index +1}}</td>
                                                        <td>
                                                            <img src="{{asset('uploads-photos/contact-client-photo')}}/{{$deleted_client->customer_photo}}" width="150px" alt="not found">
                                                        </td>
                                                        <td>
                                                            <div class="mb-2 ">
                                                                <a href="{{route('restore_contact_client',$deleted_client->id)}}" type="button" class="btn btn-success btn-square btn-xs">Restore</a><br>
                                                                <a href="{{route('contact_client.forceDelete',$deleted_client->id)}}" type="button" class="btn btn-danger btn-square btn-xs mt-2">Permanent Delete</a>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                @empty
                            ...
                            <tbody>
                                @forelse ($contact_customer_info as $contact_customer )
                                    <tr>
                                        <td>{{$loop->index +1}}</td>
                                        <td>
                                            <img src="{{asset('uploads-photos/contact-client-photo')}}/{{$contact_customer->customer_photo}}" width="150px" alt="not found">
                                        </td>
                                        <td>
                                            <div class="btn-group mb-2 ">
                                               <a href="{{route('contact_client.edit',$contact_customer->id)}}" type="button" class=""><i class="fa fa-pencil text-info fa-2x ml-3" aria-hidden="true"></i></a>
                                            </div>
                                            <form action="{{route('contact_client.destroy',$contact_customer->id)}}" method="POST">
                                                @csrf
                                                @method('delete')
